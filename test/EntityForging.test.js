...
describe('EntityForging', () => {
    ...
    before(async () => {
        ...
        const merkleInfo = (0, genMerkleTreeLib_1.default)([
            owner.address,
            user1.address,
            user2.address,
        ]);
    ...
    describe('listForForging', () => {
        it('should not allow non-owners to list a token for forging', async () => {
            ...
            await expect(entityForging.connect(user1).listForForging(tokenId, fee)).to.be.revertedWith('Caller must own the token');
        });
    ...
    describe('Forge With Listed', () => {
        ...
        it('should allow forging with a listed token', async () => {
            ...
            await expect(entityForging
                ...
                .withArgs(expectedTokenId, forgerTokenId, mergerTokenId, (forgerEntropy + mergerEntrypy) / 2n, FORGING_FEE)
                .to.emit(nft, 'NewEntityMinted')
                .withArgs(await user1.getAddress(), expectedTokenId, 2, (forgerEntropy + mergerEntrypy) / 2n);
